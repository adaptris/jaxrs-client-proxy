...
public class MetaDataReader implements Serializable{
	...
	private static final List<Class<? extends Annotation>> PARAM_ANNOTATIONS = 
			Arrays.asList(PathParam.class, QueryParam.class, HeaderParam.class, MatrixParam.class, FormParam.class);
		...
		if (httpMethod == null) {
			if (method.getAnnotations() != null) {
	            for (Annotation annotation : method.getAnnotations()) {
	            	ValuedAnnotation<String> valuedAnnotation = new ValuedAnnotation<String>(annotation.annotationType(), HttpMethod.class);
	...
	private List<NameValuePair<Integer>> readPositions(Class<? extends Annotation> annClass) {
		...
		if (parameterAnnotations != null && parameterAnnotations.length > 0) {
			...
			for (Annotation[] annotations : parameterAnnotations) {
				Optional<? extends Annotation> annotation = FluentIterable
					.of(annotations)
					.filter(annClass)
					.first();
	...
	private List<NameValuePair<Object>> readValues(Class<? extends Annotation> annotation, Object[] args) {
		...
		return readValues(positions, args);
	}
			...
			if (parametersAnnotations != null && parametersAnnotations.length > 0) {
				...
				for (Annotation[] annotations : parametersAnnotations) {
					if (!containsParamAnnotation(annotations)) {
						EntityDescription description = new EntityDescription(position, method.getParameterTypes()[position]);
